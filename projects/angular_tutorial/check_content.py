...
Angular templates make this sharper than it was for React: a post body is full of
`<app-product-card [product]="p" />` and `@if (…) { … }` inside <pre> blocks, and
one missed &lt; is invisible until it renders.

While the track is being authored, posts that have no file yet are reported as
`not written` and do not fail the run — the whole track is blocked on
pizza-angular-frontend. Once every file exists this becomes a plain pass/fail.

    python projects/angular_tutorial/check_content.py
"""
