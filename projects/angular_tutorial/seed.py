#!/usr/bin/env python3
"""Write the Angular category and its posts into a content tree.
...
build reads only the indexes.
    ...
    python projects/angular_tutorial/seed.py

    python projects/angular_tutorial/seed.py --env local --write
    python projects/angular_tutorial/seed.py --env prod  --write
...
`angular-component` was published in 2019 and therefore already carries a date
...
def read_post_html(entry: dict) -> str:
    ...
    if not path.exists():
        raise SystemExit(
            ...
            "Post bodies are written from pizza-angular-frontend. If that app is not "
            "finished yet, this track is not ready to seed — see progress_report.md."
        )
