#!/usr/bin/env python3
"""Write the Backend Dev category and its posts into a content tree.
...
build reads only the indexes.
    ...
    python projects/backend_dev/seed.py

    python projects/backend_dev/seed.py --env local --write
    python projects/backend_dev/seed.py --env prod  --write
...
Both pre-existing slugs were published on 2020-01-19 and therefore already carry
...
def main() -> int:
    ...
    parser.add_argument("--force-dates", action="store_true",
                        help="stamp the manifest date onto posts that already exist. "
                             "Needed once, to reorder the 2 rewritten 2020 posts.")
