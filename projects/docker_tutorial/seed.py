#!/usr/bin/env python3
"""Write the Docker category and its posts into a content tree.
...
build reads only the indexes.
    ...
    python projects/docker_tutorial/seed.py

    python projects/docker_tutorial/seed.py --env local --write
    python projects/docker_tutorial/seed.py --env prod  --write
...
`docker-what-is-docker` was published in 2020 and therefore already carries a date
...
track read in lesson order. It is not the default: without it a re-run leaves
the archive alone, which is the behaviour you want every other time.
"""
...
def read_post_html(entry: dict) -> str:
    ...
    if not path.exists():
        raise SystemExit(
            ...
            "Post bodies quote the Docker artifacts in lovemesomecoding_demo_project/pizza. "
            "If those are not built and verified yet, this track is not ready to seed — "
...
def main() -> int:
    ...
    parser.add_argument("--force-dates", action="store_true",
                        help="stamp the manifest date onto posts that already exist. "
                             "Needed once, to reorder the rewritten 2020 post.")
