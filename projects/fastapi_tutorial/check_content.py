...
fail the run. Once every file exists this becomes a plain pass/fail.
    ...
    python projects/fastapi_tutorial/check_content.py
"""
...
# frontend supplies the grammar, and either one alone renders the block grey. Unlike the Hasura
# track (which had to ADD `graphql`), all of these already exist; this guards against a
# regression, not a gap.
REQUIRED_LANGUAGES = {"python", "bash", "docker", "yaml", "json", "sql", "properties"}
PRISM_IMPORTS = {
    "python": "prism-python", "bash": "prism-bash", "docker": "prism-docker",
    "yaml": "prism-yaml", "json": "prism-json", "sql": "prism-sql",
...
MEASURED_STRINGS = {
    str(_M["stats_sync_median_ms"]),
    str(_M["stats_async_median_ms"]),
    _M["image_size"],
    *[str(v) for row in _M["async_crossover"] for v in row[1:]],
}

# Posts that are REQUIRED to carry the measurements, because their whole argument rests on them.
MUST_CITE = {
    "fastapi-async-vs-sync": [str(_M["stats_sync_median_ms"]), str(_M["stats_async_median_ms"])],
    "fastapi-docker": [_M["image_size"]],
}
...
for entry in manifest.POSTS:
    ...
    # (5) a rewrite has to actually shrink
    if entry["state"] == "rewrite":
        ...
        if words >= baseline:
            failures.append(
                ...
                f"{baseline} ({base_min} min) — the whole point was to cut it")
...
if written:
    ...
    print(f"\nthe nine live posts total {live_total:,} words "
