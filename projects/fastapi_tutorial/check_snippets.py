...
⚠️ THIS IS THE CHECK THAT JUSTIFIES THE WHOLE TRACK. The nine posts being replaced quote invented
`Product` and `Order` models that exist in no repository, which is exactly why they cannot be
trusted and exactly what this rewrite is for. Every python block here should be traceable to a
file that runs and is covered by StayHub's 100-test suite.
...
to start with a common line.
    ...
    python projects/fastapi_tutorial/check_snippets.py
    python projects/fastapi_tutorial/check_snippets.py --show-illustrative
"""
