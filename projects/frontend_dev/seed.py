#!/usr/bin/env python3
"""Write the Frontend Dev category and its posts into a content tree.
...
build reads only the indexes.
    ...
    python projects/frontend_dev/seed.py

    python projects/frontend_dev/seed.py --env local --write
    python projects/frontend_dev/seed.py --env prod  --write
...
Both pre-existing slugs were published on 2019-02-06 and therefore already carry
...
def main() -> int:
    ...
    parser.add_argument("--force-dates", action="store_true",
                        help="stamp the manifest date onto posts that already exist. "
                             "Needed once, to reorder the 2 rewritten 2019 posts.")
