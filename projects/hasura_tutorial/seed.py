#!/usr/bin/env python3
"""Write the Hasura category and its posts into a content tree.
...
build reads only the indexes.
    ...
    python projects/hasura_tutorial/seed.py

    python projects/hasura_tutorial/seed.py --env local --write
    python projects/hasura_tutorial/seed.py --env prod  --write
...
⚠️ ELEVEN of the twenty slugs were published in early 2020 and therefore already
carry dates that upsert_post will not overwrite. `--force-dates` stamps the
manifest date onto an existing post before upserting it, which is what makes the
rewritten track read in lesson order. Without it the archive interleaves nine
2026 posts with eleven 2020 ones and the pager reads nonsense.

It is not the default: without it a re-run leaves the archive alone, which is the
behaviour you want every other time. Expect to pass it exactly once per tree.
"""
...
def read_post_html(entry: dict) -> str:
    ...
    if not path.exists():
        raise SystemExit(
            ...
            "v2 snippets are quoted from the running StayHub stack in "
            "lovemesomecoding_demo_project/stayhub. If the engine is not up and the four "
            "additions (Action, event trigger, subscription, insert permission) are not "
            "built, this track is not ready to seed — see progress_report.md."
        )
...
def main() -> int:
    ...
    parser.add_argument("--force-dates", action="store_true",
                        help="stamp the manifest date onto posts that already exist. "
                             "Needed once, to reorder the eleven rewritten 2020 posts.")
    ...
    # A frozen slug that is missing from the target tree means the URL this
    # rewrite is meant to land on does not exist there. Harmless on `local`,
    # which is seeded from a copy — but on `prod` it means the slug is wrong,
    # and upserting would mint a NEW url rather than rewrite the indexed one.
    #
    # ⚠️ This runs BEFORE the post bodies are read, deliberately. While the track
    # is being authored every file is missing, and a guard placed after the file
    # check is one that never runs until the day it finally matters.
    if not args.only:
        ...
        if absent:
            ...
            if args.env == "prod":
                raise SystemExit(
                    message + "\nThese are supposed to be the indexed 2020 URLs being "
                    "rewritten in place. Seeding would create new pages instead. "
                    "Check the slugs before writing to prod.")
    ...
    for entry in posts:
        ...
        print(f"  {record['url']:46} {record['date'][:10]}  "
