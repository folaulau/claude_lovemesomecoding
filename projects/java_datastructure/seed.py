#!/usr/bin/env python3
"""Write the Data Structures & Algorithms category and its posts into a content tree.
...
build reads only the indexes.
    ...
    python projects/java_datastructure/seed.py

    python projects/java_datastructure/seed.py --env local --write
    python projects/java_datastructure/seed.py --env prod  --write
...
All 24 pre-existing slugs were published between 2018 and 2019 and therefore
already carry a date that upsert_post will not overwrite. `--force-dates` stamps
the manifest date onto an existing post before upserting it, which is what makes
the rewritten track read in reading order. It is not the default: without it a
re-run leaves the archive alone, which is the behaviour you want every other time.
"""
...
def main() -> int:
    ...
    parser.add_argument("--force-dates", action="store_true",
                        help="stamp the manifest date onto posts that already exist. "
                             "Needed once, to reorder the 24 rewritten 2018-2019 posts.")
