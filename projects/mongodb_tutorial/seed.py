#!/usr/bin/env python3
"""Write the MongoDB category and its posts into a content tree.
...
posts, and the static build reads only the indexes.
    ...
    python projects/mongodb_tutorial/seed.py

    python projects/mongodb_tutorial/seed.py --env local --write
    python projects/mongodb_tutorial/seed.py --env prod  --write
...
Two situations need it. The first is the original seed: three of the sixteen slugs were published
in July 2019 and already carry dates, so without it the archive interleaves three 2019 posts with
thirteen 2024-25 ones and the pager reads nonsense. The second is any re-base of START_DATE — once
a post is published its stored date is sticky, so moving the whole track means --force-dates again.
...
def read_post_html(entry: dict) -> str:
    ...
    if not path.exists():
        raise SystemExit(
            ...
            "Every code sample in this track is quoted from the ReelCMS app in "
            f"{manifest.DEMO_APP}. Run check_content.py and check_snippets.py before seeding — "
            "see progress_report.md."
        )
    return path.read_text(encoding="utf-8")


def assert_dates_in_window() -> None:
    """Folau's constraint: every post date must fall in 2024-2025.

    Checked here as well as in check_content.py because seeding is the step that actually makes a
    date public, and a re-based START_DATE is exactly the kind of edit that slips past a check
    nobody re-ran.
    """
    low, high = manifest.DATE_WINDOW
    stray = [
        f"{entry['slug']} ({entry['date'][:10]})"
        for entry in manifest.POSTS
        if not (low <= datetime.fromisoformat(entry["date"]) <= high)
    ]
    if stray:
        raise SystemExit(
            "post dates outside the agreed 2024-2025 window: " + ", ".join(stray) +
            "\nAdjust START_DATE / STEP_DAYS in manifest.py."
        )
...
def main() -> int:
    ...
    parser.add_argument("--force-dates", action="store_true",
                        help="stamp the manifest date onto posts that already exist. "
                             "Needed once, to re-base the three 2019 posts into 2024-2025.")
    ...
    if not args.only:
        ...
        if absent:
            ...
            if args.env == "prod":
                raise SystemExit(
                    message + "\nThese are supposed to be the indexed 2019 URLs being rewritten "
        ...
        # upserting it would move a stranger's page into /mongodb and rewrite its body. So the
    ...
    if over:
        print(f"\n⚠️  {len(over)} post(s) over the {manifest.TARGET_MINUTES[1]}-minute budget: "
              + ", ".join(over))
