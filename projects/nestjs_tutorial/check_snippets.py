#!/usr/bin/env python3
"""Prove every code sample still matches the contractor file it was copied from.
...
⚠️ THE MATCH RATE MEANS THE OPPOSITE OF WHAT IT MEANS IN THE TYPESCRIPT TRACK. Read this before
judging the number.

That track is a LANGUAGE tutorial: most of its blocks are three lines demonstrating one rule, they
belong in no repository, and a match rate of 38% is correct there. This is a FRAMEWORK tutorial.
A Nest lesson quotes a whole guard, a whole module, a whole DTO — the shape of the file IS the
lesson — so nearly every block should match the app, and a LOW rate is the finding. MIN_MATCH_SHARE
below turns that into a check rather than a hope, and it only fires once the whole track is
written.

The near-miss detector matters just as much as it does anywhere else: a block whose opening lines
ARE in the app but whose body is not is the signature of a real quote that has gone stale, and it
...
comments (the house style quotes the app verbatim MINUS its teaching comments — the contractor
files carry long ⚠️ blocks, and repeating them in a post that is explaining the same thing in
prose would say everything twice). A line that is exactly `...` or `// ...` marks a deliberate
elision and splits the block into chunks that must each match, though they need not be adjacent.
...
is reported.
    ...
    python projects/nestjs_tutorial/check_snippets.py
    python projects/nestjs_tutorial/check_snippets.py --show-illustrative
"""
...
SKIP_DIRS = {"node_modules", "dist", "build", ".git", ".angular", "coverage", ".vite",
             ...
             # the contractor API writes portfolio photos here at runtime
             "uploads"}
...
# against file contents would report the whole track as drift. `sql` likewise — the SQL in this
# track is quoted from inside migration strings and from psql sessions, not from .sql files.
# `tsx` is absent because this is a backend track; there is no JSX anywhere in it.
SOURCE_LANGS = {"typescript", "json"}
...
# The share of checked blocks that must be quoted from the app once the track is complete.
#
# 70% rather than something higher, because a real Nest lesson does contain a handful of blocks
# that legitimately match nothing: a `nest g resource` invocation's OUTPUT, a hand-written
# `useFactory` shown to explain provider tokens before the app's own is introduced, the shape of a
# response body. Those are teaching scaffolding, not quotes. What 70% rules out is a post built
# mostly of invented code while claiming otherwise.
MIN_MATCH_SHARE = 0.70
...
#
# ⚠️ This track uses the marker RARELY, and that is deliberate. In a framework tutorial an
# unmatched block is a stale quote by default, so every use of the marker is a hole punched in the
# only check that catches drift. Reach for it only for the genuine article: the plausible thing the
# app deliberately does not do. check_content.py enforces the other direction — a block that shows
# the wrong way and is NOT marked gets a warning there.
...
def main() -> int:
    ...
    share = matched / checked if checked else 0
    if checked:
        print(f"{matched}/{checked} checked blocks ({share:.0%}) are quoted from code that runs")

    # ⚠️ The check that is specific to a FRAMEWORK track, and the inverse of the TypeScript one.
    #
    # There, a low match rate is expected and meaningless. Here the track's own claim is that
    # "every example is lifted from a working contractor marketplace API" — so a low rate means
    # that claim has stopped being true, either because posts drifted into invented examples or
    # because SNIPPET_SOURCES stopped pointing at what the posts actually quote.
    #
    # Only enforced once every post exists: a half-written track's rate says nothing.
    complete = written == len(manifest.POSTS)
    below_floor = complete and checked and share < MIN_MATCH_SHARE
    if below_floor:
        print(f"\n⚠️  {share:.0%} is below the {MIN_MATCH_SHARE:.0%} floor for this track. In a "
              "framework tutorial the blocks ARE the app;\n    a low share means the posts have "
              "drifted into invented examples, or SNIPPET_SOURCES no longer\n    names the files "
              "they quote. Run with --show-illustrative to see which blocks matched nothing.")
