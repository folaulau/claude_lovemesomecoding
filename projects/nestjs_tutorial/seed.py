#!/usr/bin/env python3
"""Write the NestJS category and its posts into a content tree.
...
posts, and the static build reads only the indexes.
    ...
    python projects/nestjs_tutorial/seed.py

    python projects/nestjs_tutorial/seed.py --env local --write
    python projects/nestjs_tutorial/seed.py --env prod  --write
...
⚠️ And the frontend nav is a SEPARATE step this script cannot do. `nestjs` is a new category slug,
so the JavaScript dropdown will not list it however many times you seed — it needs adding to
lovemesomecoding_frontend/src/lib/nav.ts, where `navTree()` filters against `allCategories()`.
Done 2026-09-05; check_content.py warns if it is ever removed. See progress_report.md.
"""
...
def read_post_html(entry: dict) -> str:
    ...
    if not path.exists():
        raise SystemExit(
            ...
            "Every code sample in this track is quoted from the contractor marketplace API in "
            "lovemesomecoding_demo_project/contractor/contractor-nestjs-backend. Run "
...
def main() -> int:
    ...
    if not args.only:
        ...
        # /nestjs), which makes this a no-op; it stays wired up because the day someone
        # publishes a nestjs post by hand, adding the slug to the manifest is the whole fix.
        #
        ...
        # would move a stranger's page into /nestjs and rewrite its body. So the check is on
    ...
    print("\n⚠️  the nav entry for `nestjs` lives in lovemesomecoding_frontend/src/lib/nav.ts "
          "(JavaScript group)\n    and seeding cannot create it. Added 2026-09-05 — "
          "check_content.py warns if it goes missing.")
    return 0
