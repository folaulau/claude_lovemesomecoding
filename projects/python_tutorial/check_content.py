...
2. **The gates this track exists for.** Unlike /java, the prose on /python was
   already the right length. The bloat is CODE: 9 to 53 blocks per post, four
   posts with no headings at all and therefore an empty table of contents. So
   CODE_BLOCK_MAX and H2_RANGE are the checks that bite here, not WORD_TARGET.

No AWS access needed.

    python3 projects/python_tutorial/check_content.py
"""
...
for entry in manifest.POSTS:
    ...
    # THE check for this track. `python-modules-packages` shipped 53 blocks and
    # `python-lists-list-comprehensions` 41 on 966 words of prose. For
    # calibration the 29 authored /java posts run 2-12 blocks, mean 8.
    ...
    # Four posts currently have NO headings and an empty TOC; two others carry
    # 53-entry ones. Both are the same bug seen from opposite ends.
    ...
    if len(tags) < 2:
        failures.append(f"{entry['slug']}: {len(tags)} tag(s) — all 17 old posts had "
