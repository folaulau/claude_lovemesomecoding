...
pizza-react-native-mobile. Indentation is ignored, because a fragment lifted out of
deeply nested JSX is dedented when quoted, and that is fine. A line that is
exactly `...` marks a deliberate elision and splits the block into chunks that
...
drifted quote looks like.
    ...
    python projects/react_native/check_snippets.py
    python projects/react_native/check_snippets.py --show-illustrative
...
APP = HERE.parent.parent / "lovemesomecoding_demo_project/pizza/pizza-react-native-mobile"
...
# Everything under APP is scanned EXCEPT these. The project root has to be in
# scope, not just src/ — posts quote app.config.ts, package.json and tsconfig.json,
# and scanning only src/ silently reports those as "illustrative" rather than
# verifying them. `ios/` and `android/` are generated by `expo prebuild` and are
# not authored code.
SKIP_DIRS = {"node_modules", "dist", ".expo", ".git", "ios", "android", "test-results",
             "playwright-report", "screenshots", "coverage", "web-build"}
...
# Languages that could plausibly have come out of the app at all. `tsx` carries
# most of it — this is a React Native codebase, so nearly every component file is
# TSX rather than plain TypeScript. Both are already supported by the content
# normaliser and by the frontend's Prism imports; no pipeline change was needed.
SOURCE_LANGS = {"tsx", "jsx", "typescript", "json"}
...
# ⚠️ Each block-comment pattern is anchored to the START OF A LINE, and that is
# load-bearing rather than tidy. An unanchored `/\*.*?\*/` also matches the `/*`
# inside the string `"@/*": ["./src/*"]` in tsconfig.json, and then runs forward to
# the next real `*/` — swallowing the whole `paths` block. The symptom is a snippet
# quoted verbatim from the app being reported as `illustrative`, which reads as
# "invented example" rather than "the stripper ate it". Every block comment in this
# codebase begins its own line, so anchoring costs nothing.
COMMENTS = [
    re.compile(r"^[ \t]*\{/\*.*?\*/\}[ \t]*$", re.S | re.M),  # {/* ... */} JSX comments
    re.compile(r"^[ \t]*/\*.*?\*/", re.S | re.M),               # /* ... */ and /** ... */
...
def strip_comments(text: str) -> str:
    ...
    Button.tsx is the imports followed immediately by the component, while the
    file has a 12-line comment between them. Without this, every such quote reads
    as drift.
    """
...
def main() -> int:
    ...
    for path in APP.rglob("*"):
        if not path.is_file() or path.suffix not in {".ts", ".tsx", ".js", ".jsx", ".json"}:
            continue
        # A 500 KB lockfile is a megabyte of noise no post will ever quote.
    ...
    for entry in manifest.POSTS:
        ...
        for lang, raw in BLOCK.findall(path.read_text(encoding="utf-8")):
            ...
            # "import { View } from 'react-native';" opens half the app and every
            # invented example alike.
