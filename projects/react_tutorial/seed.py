#!/usr/bin/env python3
"""Write the React category and its posts into a content tree.
...
build reads only the indexes.
    ...
    python projects/react_tutorial/seed.py

    python projects/react_tutorial/seed.py --env local --write
    python projects/react_tutorial/seed.py --env prod  --write
...
17 of these slugs were published in 2019 and therefore already carry a date that
upsert_post will not overwrite. `--force-dates` stamps the manifest date onto an
existing post before upserting it, which is what makes the rewritten track read
in lesson order. It is not the default: without it a re-run leaves the archive
alone, which is the behaviour you want every other time.
"""
...
def main() -> int:
    ...
    parser.add_argument("--force-dates", action="store_true",
                        help="stamp the manifest date onto posts that already exist. "
                             "Needed once, to reorder the 17 rewritten 2019 posts.")
