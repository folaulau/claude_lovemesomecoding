#!/usr/bin/env python3
"""Write the Spring Study Guide category and its posts into a content tree.
...
build reads only the indexes.
    ...
    python projects/spring_study_guide/seed.py

    python projects/spring_study_guide/seed.py --env local --write
    python projects/spring_study_guide/seed.py --env prod  --write
...
All 8 pre-existing slugs were published on 2019-08-21 and therefore already carry
a date that upsert_post will not overwrite. `--force-dates` stamps the manifest
date onto an existing post before upserting it, which is what makes the rewritten
track read in reading order. It is not the default: without it a re-run leaves the
archive alone, which is the behaviour you want every other time.
"""
...
def main() -> int:
    ...
    parser.add_argument("--force-dates", action="store_true",
                        help="stamp the manifest date onto posts that already exist. "
                             "Needed once, to reorder the 8 rewritten 2019 posts.")
