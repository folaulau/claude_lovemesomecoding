#!/usr/bin/env python3
"""Write the Spring Boot category and its posts into a content tree.
...
build reads only the indexes.
    ...
    python projects/springboot_tutorial/seed.py

    python projects/springboot_tutorial/seed.py --env local --write
    python projects/springboot_tutorial/seed.py --env prod  --write
...
All 31 pre-existing slugs were published between 2018 and 2023 and therefore
...
the rewritten track read in lesson order. It is not the default: without it a
...
def main() -> int:
    ...
    parser.add_argument("--force-dates", action="store_true",
                        help="stamp the manifest date onto posts that already exist. "
                             "Needed once, to reorder the 31 rewritten 2018-2023 posts.")
