...
⚠️ THIS IS THE CHECK THAT JUSTIFIES THE WHOLE TRACK. The seven posts being replaced contain two
code blocks between them and thirty-five screenshots — nothing a reader can run, check or
disagree with. Every python block here should be traceable to a file that runs and is covered by
StayHub's 165-test suite.

⚠️ AND THE EMPTY LISTS ARE THE POINT OF THE OTHER HALF. `system-design-amazon`,
`system-design-delta-airlines`, `system-design-url-shortener` and `system-design-chat-system`
declare NO sources, because they are not StayHub — their schemas and flows are design work, not a
running application. Everything they show must therefore be SQL, a diagram, or a sketch that is
labelled as one. What must never happen is Python in a system-design post that reads like it came
out of a repository when it did not; that is precisely the failure the FastAPI rewrite existed to
correct, and it would be worse here because the case studies are the posts people trust most.
...
to start with a common line.
    ...
    python projects/system_design/check_snippets.py
    python projects/system_design/check_snippets.py --show-illustrative
"""
...
APP = HERE.parent.parent / "lovemesomecoding_demo_project/stayhub/stayhub-fastapi-backend"
...
# ⚠️ `[^>]*` after the class, and it is load-bearing.
#
# The first version required `<pre class="language-X">` EXACTLY, which meant a block carrying any
# extra attribute — such as this track's `data-antipattern` marker — did not match at all. Those
# blocks were therefore invisible to this checker rather than excluded by it: not counted, not
# compared, and silently absent from the totals. A genuinely drifted quote could have hidden behind
# any stray attribute.
#
# Found on 2026-08-22, when post 9 reported four antipattern blocks in the source and one in the
# report. The published HTML was never affected — content.normalize() strips the attribute, so the
# marker is author-only metadata that never ships.
BLOCK = re.compile(
