#!/usr/bin/env python3
"""Write the System Design category and its posts into a content tree.
...
posts, and the static build reads only the indexes.
    ...
    python projects/system_design/seed.py

    python projects/system_design/seed.py --env local --write
    python projects/system_design/seed.py --env prod  --write
...
⚠️ SEVEN of the eighteen slugs have been live since 2018/2019/2023 and therefore already carry
dates that upsert_post will not overwrite. `--force-dates` stamps the manifest date onto an
existing post before upserting it, which is what makes the rewritten track read in lesson order.
Without it the archive interleaves eleven 2026 posts with seven much older ones and the pager
reads nonsense.
...
def read_post_html(entry: dict) -> str:
    ...
    if not path.exists():
        raise SystemExit(
            ...
            "Every code sample in this track is quoted from the StayHub backend in "
...
def main() -> int:
    ...
    if not args.only:
        ...
        if absent:
            ...
            if args.env == "prod":
                raise SystemExit(
                    message + "\nThese are supposed to be the indexed legacy URLs being rewritten "
