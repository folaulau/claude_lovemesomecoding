...
A Terraform track makes this sharper still. Post bodies quote HCL that is almost
entirely `resource "x" "y" { … }` braces, `${var.name}` interpolation, heredocs,
JSON IAM policy documents, and `>` / `<` in version constraints like
`version = ">= 6.0"` — all inside <pre> blocks, and one missed &gt; is invisible
until it renders.

⚠️ This checks that the HTML round-trips. It does NOT check that the Terraform is
valid — that is check_hcl.py, which runs `terraform fmt -check` and `terraform
validate` over every HCL block. Run both; only the latter goes stale on its own
when a provider renames an argument.
...
plain pass/fail.
    ...
    python projects/terraform_tutorial/check_content.py
"""
...
# manifest._date() already raises if a computed date leaves the window, so by the
# time this module imports it the rule holds. Re-assert it here anyway against the
# built POSTS list, because that is the list seed.py actually writes and it is
# cheap to prove rather than infer.
_lo, _hi = manifest.DATE_WINDOW
for entry in manifest.POSTS:
    if not _lo.strftime("%Y-%m-%dT%H:%M:%S") <= entry["date"] <= _hi.strftime("%Y-%m-%dT%H:%M:%S"):
        failures.append(
            f"{entry['slug']}: date {entry['date'][:10]} is outside the allowed window "
            f"{_lo:%Y-%m-%d}..{_hi:%Y-%m-%d}")
