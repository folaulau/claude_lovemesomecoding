#!/usr/bin/env python3
"""Write the Terraform category and its posts into a content tree.
...
build reads only the indexes.
    ...
    python projects/terraform_tutorial/seed.py

    python projects/terraform_tutorial/seed.py --env local --write
    python projects/terraform_tutorial/seed.py --env prod  --write
...
All three of `terraform-introduction`, `terraform-fundamentals` and
`terraform-with-aws` were published 2019-07-09 and therefore already carry dates
that upsert_post will not overwrite. `--force-dates` stamps the manifest date
onto an existing post before upserting it, which is what makes the rewritten
track read in lesson order — without it lessons 1, 2 and 12 sort to the BACK of
the archive, which is exactly backwards.

Dates in this track are computed from START_DATE, so a re-base is an intended
operation and `--force-dates` is whatever a manifest date change requires. This
is the FastAPI track's position, not the AWS track's: there, dates are
...
def read_post_html(entry: dict) -> str:
    ...
    if not path.exists():
        raise SystemExit(
            ...
            "Post bodies quote the Terraform in projects/terraform_tutorial/infra, which is "
            "applied to a real AWS account and destroyed before being written about. If that "
            "has not happened yet, this track is not ready to seed — see progress_report.md."
        )
...
def main() -> int:
    ...
    parser.add_argument("--force-dates", action="store_true",
                        help="stamp the manifest date onto posts that already exist. "
                             "Needed to reorder the three rewritten 2019 posts.")
