...
TypeScript sharpens that in its own way. A generic is written `Array<[Unit, number]>` and a JSX
prop type is written `<T,>(…)`; both are full of angle brackets that an HTML parser is entitled to
read as tags. A block that loses `<T>` still looks like TypeScript, and `Promise<T>` silently
becoming `Promise` is exactly the kind of damage a length check waves through.
...
On top of the round-trip check this enforces the rules specific to this track:
  ...
  2. Prose is at least 45% of the words — higher than the FastAPI track's 40%, because a language
     tutorial's snippets are short. See manifest.MIN_PROSE_SHARE for why that follows.
  3. No block renders as `plaintext`. Every language this track uses is supported on both halves,
     so a plaintext block is a typo in a class attribute, not a gap — and it fails silently,
     because an unsupported language is normalised rather than rejected.
  4. A block demonstrating a compile ERROR sits near manifest.ANTIPATTERN_MARKER. Half of teaching
     a type system is showing the code it rejects; none of that code is in the app, so without the
     marker check_snippets.py files it under `illustrative` and the drift signal gets diluted by
     the posts doing their job properly.
...
fail the run. Once every file exists this becomes a plain pass/fail.
    ...
    python projects/typescript_tutorial/check_content.py
"""
...
# frontend supplies the grammar, and either one alone renders the block grey. All five already
# exist (verified 2026-09-05), so this guards against a regression rather than filling a gap.
REQUIRED_LANGUAGES = {"typescript", "tsx", "json", "bash", "markup"}
PRISM_IMPORTS = {
    "typescript": "prism-typescript", "tsx": "prism-tsx", "json": "prism-json",
    "bash": "prism-bash", "markup": "prism-markup",
}

# Rule 4. What a block demonstrating a rejected program looks like.
#
# The house style for these is a comment carrying the compiler's own words, because paraphrasing
# an error message teaches the reader to search for a string that does not exist.
COMPILE_ERROR = re.compile(
    r"(is not assignable to|Object is possibly|has no properties in common|"
    r"does not exist on type|Property '[^']+' is missing|"
    r"ts\(\d{4}\)|// Error[: ]|// ✗)", re.I)
...
for entry in manifest.POSTS:
    ...
    #
    # ⚠️ The distinction is the whole rule. `plaintext` is the right language for compiler
    # diagnostics — a `tsc` error message is not TypeScript and Prism has no grammar for it — so
    # banning it outright was wrong. But an unsupported language is NORMALISED to plaintext rather
    # than rejected, so `language-typescrpt` also lands here, silently, rendering grey.
    #
    # Comparing what the author WROTE against what the pipeline produced separates the two.
    ...
    # (4) an error demonstration has to be marked as one
    #
    # Only for blocks in a SOURCE language. A `plaintext` block quoting a compiler diagnostic is
    # error text by definition, and check_snippets.py never tries to match it against the app, so
    # there is nothing for the marker to fix.
    for i, match in enumerate(OUT_PRE.finditer(body)):
        if match.group(1) not in ("typescript", "tsx", "json"):
            continue
        block_text = html.unescape(match.group(2))
        if not COMPILE_ERROR.search(block_text):
            continue
        ...
        if not (ANTIPATTERN.search(window) or ANTIPATTERN.search(match.group(0))):
            warnings.append(
                f"{entry['slug']} block {i}: shows a compiler error but no "
                f"{manifest.ANTIPATTERN_MARKER!r} within {MARKER_WINDOW} chars before it — "
                "check_snippets.py will treat it as an unmatched quote")
    ...
    # (2) the prose floor — the anti-syntax-dump rule
    if share < manifest.MIN_PROSE_SHARE:
        failures.append(
            ...
            "That is a syntax reference with captions, not a lesson.")
...
# FROZEN_SLUGS is empty today and that is correct — nothing is published under /typescript. But
...
if prism_file.exists():
    ...
    for lang in sorted(REQUIRED_LANGUAGES):
        # prism-markup is built into prismjs core and imported by no line of its own.
        if lang == "markup":
            continue
...
# The nav. This track's category is brand new, so unlike every other track the dropdown entry is a
# real task — and one that is invisible when forgotten: the posts publish and resolve perfectly,
# they are just unreachable from the menu.
nav_file = HERE.parent.parent / "lovemesomecoding_frontend/src/lib/nav.ts"
if nav_file.exists():
