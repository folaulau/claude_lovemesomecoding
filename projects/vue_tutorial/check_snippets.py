...
reelcms-vue-frontend. Indentation is ignored, because a fragment lifted out of
...
drifted quote looks like.
    ...
    python projects/vue_tutorial/check_snippets.py
    python projects/vue_tutorial/check_snippets.py --show-illustrative
...
APP = HERE.parent.parent / "lovemesomecoding_demo_project/reelcms/reelcms-vue-frontend"

# Everything under APP is scanned EXCEPT these. The project root has to be in
# scope, not just src/ — posts quote vite.config.js and package.json, and scanning
# only src/ silently reported those as "illustrative" rather than verifying them.
SKIP_DIRS = {"node_modules", "dist", ".git", "test-results",
             "playwright-report", "screenshots", ".vscode"}
...
#
# `vue` is the important one: a single-file component is the unit of work in Vue,
# so most quotable snippets in this track are a whole .vue file or a section of
# one. Leaving it out would silently skip the majority of the track's code.
SOURCE_LANGS = {"vue", "javascript", "markup", "css", "scss", "json"}
...
COMMENTS = [
    re.compile(r"/\*.*?\*/", re.S),        # /* ... */, including ReelCMS's teaching blocks
...
def main() -> int:
    ...
    for entry in manifest.POSTS:
        ...
        for lang, raw in BLOCK.findall(path.read_text(encoding="utf-8")):
            ...
            # `import { ref } from "vue";` opens half the app and every invented
            # example alike.
